...
import { loadGlobalOrder, storeGlobalOrder } from "./global-order-storage";
...
async function handleTaskCreation(task: Task): Promise<void> {
    try {
        ...
        // Load current order and it will auto-sync to include the new task
        const order = await loadGlobalOrder(workstreamUuids, taskUuids);

        // Order is automatically saved by loadGlobalOrder during sync
...
async function handleTaskDeletion(taskId: string): Promise<void> {
    try {
        ...
        // Load current order and it will auto-sync to remove the deleted task
        const order = await loadGlobalOrder(workstreamUuids, taskUuids);
...
async function handleWorkstreamCreation(workstream: Workstream): Promise<void> {
    try {
        ...
        // Load current order and it will auto-sync to include the new workstream
        const order = await loadGlobalOrder(workstreamUuids, taskUuids);
...
async function handleWorkstreamDeletion(workstreamId: string): Promise<void> {
    try {
        ...
        // Load current order and it will auto-sync to remove the deleted workstream
        const order = await loadGlobalOrder(workstreamUuids, taskUuids);
